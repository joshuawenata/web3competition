...
export default function Home() {
    ...
    const fetchHaki = async () => {
        try {
            const response = await fetch("http://localhost:4000/fetch-your-haki", {
                method: 'GET',
                headers: {
    ...
    const [text, setText] = useState({jenis : 'copyright type', negara : 'country', kota: 'city', tanggal: '', status: 'status'});
    let itemsPerPage = 10;
    let number = 0;
    ...
    const handleFilter = () => {
        if (!filterEnabled) {
            setFilter(true)
            setText({jenis : 'copyright type', negara : 'country', kota: 'city', tanggal: '', status: 'status'})
        }
        else {
            setFilter(false)
            setText({jenis : 'copyright type', negara : 'country', kota: 'city', tanggal: '', status: 'status'})
